-- ============================================================
-- COPARENTALITÉ ZEN — INSTALLATION COMPLÈTE SUPABASE (v2, IDEMPOTENT)
-- Peut être exécuté et RELANCÉ sans risque : if not exists,
-- create or replace, drop policy if exists, seed anti-duplication.
...
-- ---------- Buckets privés (rejouable : on conflict do nothing) ----------
...
select 'INSTALLATION TERMINÉE' as statut,
  ...
  (select count(*) from expense_categories where household_id is null) as categories,
  (select count(*) from plans) as plans,
  (select count(*) from school_holidays) as vacances,
